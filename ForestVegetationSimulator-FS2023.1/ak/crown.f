...
C  IF THERE ARE NO LIVE TREES ON INVENTORY POINT, SET QMD OF
C  INVENTORY POINT TO 1. THE VALUE OF 1 SHOULD ONLY EVER BE ASSUMED
C  HERE IF THERE ARE NO LIVE TREES LEFT IN THE STAND.
      BAPLT = PTBAA(ITRE(I))
      TPAPLT = PTPA(ITRE(I))
...
C  CALCULATE RELATIVE DENSITY OF INVENTORY POINT
...
C  CR = 1/(1 + EXP(X)))
C  X = B1 + B2*HDR + B3*PRD + B4*DBH/QMDPLT
...
C
C  SAME RELATIONSHIP IS USED TO CALCULATE CROWN RATIO FOR DEAD
C  TREES.
C----------
...
C  IF THERE ARE NO LIVE TREES ON INVENTORY POINT, SET QMD OF
C  INVENTORY POINT TO 1. THE VALUE OF 1 SHOULD ONLY EVER BE ASSUMED
C  HERE IF THERE ARE NO LIVE TREES IN THE STAND.
      BAPLT = PTBAA(ITRE(I))
      TPAPLT = PTPA(ITRE(I))
      IF(TPAPLT .GT. 0.0) THEN
        QMDPLT = SQRT((BAPLT/TPAPLT)/0.005454)
