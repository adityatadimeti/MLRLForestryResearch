...
C----------
C  DG10 = 10-YR OUTSIDE BARK DIAMETER GROWTH, 
C         USED IN UPDATED RW EQ (JULY 2021)
C----------
...
C--------
C  CALCULATE HTG FOR REDWOOD USING THE FOLLOWING
C  FUNCTIONAL FORM
C
C  HI = EXP(X)
C  X = B1 + D^2 + LOG(D) + LOG(SI) + LOG(DGLT) + LOG(H)
C
C  WHERE
...
C  BEGIN PROCESSING OF ALL OTHER SPECIES IN
        CASE DEFAULT
...
C  IF IT IS NOT, THEN LET HTG BE COMPUTED THE NORMAL
...
C     TO BE IN (0-1) RANGE; DIVIDE BY 100.  FUNCTION IS HOERLS
